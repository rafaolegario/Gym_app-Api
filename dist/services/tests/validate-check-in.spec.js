...
// src/services/tests/validate-check-in.spec.ts
...
describe("CheckIn validate Service", () => {
  ...
  it("should be able to validate check In", async () => {
    ...
    globalExpect(checkIn.is_validated).toEqual(globalExpect.any(Date));
    globalExpect(CheckInRepository.checkIns[0].is_validated).toEqual(globalExpect.any(Date));
  });
  ...
  it("Should not be able to validate a check-in after 20 minutes of its creation", async () => {
    ...
    vi.advanceTimersByTime(1e3 * 60 * 21);
    globalExpect(async () => {
